		...
		for (int i = 0; i < TowerDefense.getWidth() / Gameplay.SIZE; ++i) {
			for (int j = 0; j < TowerDefense.getHeight() / Gameplay.SIZE; ++j) {
				if (i < this.horizontalTiles && j < this.verticalTiles) {
					this.picture.draw(i * Gameplay.SIZE- Gameplay.getCameraX(), j * Gameplay.SIZE- Gameplay.getCameraY(), Gameplay.GLOBAL_GAME_SCALE);
				} else {
					this.pictureOutOfBounds.draw(i * Gameplay.SIZE- Gameplay.getCameraX(), j * Gameplay.SIZE- Gameplay.getCameraY(), Gameplay.GLOBAL_GAME_SCALE);
				}
			}
		}
